...
Bundle a short (~1.2 sec) two-tone descending airplane-style chime as
`Sources/Multiharness/Resources/agent-ding.wav`. The asset is generated
programmatically by `scripts/generate-ding.py`, which renders pure sine
waves (fundamental + 2nd/3rd harmonics for bell-like timbre) with a fast
attack and exponential-decay envelope to 16-bit mono PCM at 22050 Hz.

The two tones are A5 (880 Hz, 450 ms) and E5 (659.25 Hz, 700 ms) with a
40 ms gap — a familiar descending "bing-bong" shape that reads as
airplane-PA-style without imitating any specific carrier.

Why synthesize rather than source: pure sine waves rendered to PCM samples
have no copyright (the audio is a deterministic function of the script), so
licensing is moot. We can also tweak tone, decay, and duration in source
control without juggling binary assets.

`Sources/Multiharness/Resources/CREDITS.txt` documents the synthesis
approach and how to regenerate.
...
Asset constraints (current generator output: 52 KB, 1.19 s):

- WAV (PCM), 16-bit, mono, ≤2 seconds, ≤100 KB.
...
- `scripts/generate-ding.py` — chime generator (Python stdlib only).
- `Sources/Multiharness/Resources/agent-ding.wav` — new asset (binary,
  generated by the script above).
- `Sources/Multiharness/Resources/CREDITS.txt` — synthesis notes + regen
  instructions.
