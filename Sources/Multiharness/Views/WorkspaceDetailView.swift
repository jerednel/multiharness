...
private struct ConversationView: View {
    ...
    /// Tracks the id of the bottom-most item so we can scroll to it.
    /// Updated on turn count changes and streaming text growth.
    @State private var bottomID: String?
    /// Whether the user has manually scrolled away from the bottom.
    /// When true we stop auto-scrolling until new user input or a new
    /// agent run resets it.
