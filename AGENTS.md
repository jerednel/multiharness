...
- `anthropic-oauth` — Codex Pro/Max OAuth. Sidecar's `oauthStore.ts` persists refresh+access tokens at `<dataDir>/oauth/anthropic.json` (mode 0600) and refreshes ~5 min before expiry.
